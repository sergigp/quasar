...
class AsyncCommandBus[C <: Command](logger: Logger) extends CommandBus[Future, C] {
  ...
  private var handlers = Map.empty[Class[_], C => Future[Unit]]

  override def publish(command: C): Future[Unit] =
    handlers
  ...
  override def subscribe[H <: C: ClassTag](handler: H => Future[Unit]): Unit = {
    ...
    synchronized {
      if (handlers.contains(classTag.runtimeClass)) {
        ...
        val transformed: C => Future[Unit] = (t: C) => handler(t.asInstanceOf[H])
